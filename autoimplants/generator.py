...
cortex itself, station by station, offset outward by the mounting clearance, so
the part follows both the longitudinal bow and the transverse curvature of the
shaft.

The generic off-the-shelf part it replaced -- a flat, straight, constant-thickness
slab -- cannot pass on this patient. A flat face has to clear the apex of the
22 mm anterior bow to stay out of the shaft, and then gapes everywhere else:
8.60 mm at the plate edge against a 1.5 mm conformance limit. No scalar version
of it closes that, because the gap is the bow plus the transverse sagitta of a
~10 mm radius cortex under a 16 mm chord, and neither term is a parameter.
...
extra radial standoff along the length on top of the fitted seating surface.
``thickness_profile``, ``ribs`` and ``hole_slots`` are still declared and still
raise, because setting a parameter must not be enough -- to use one you have to
write the geometry.
"""
...
def _seating_surface(zs: np.ndarray, lanes: np.ndarray) -> np.ndarray:
    ...
    primitive fitted through it. Fitting one circle per station would be two
    degrees of freedom against a whole cross-section, and the residual it could
    not follow would show up directly in ``bone_conformance_gap``, and then, to
    stay out of the bone, the circle would have to enclose that residual, spending
    standoff on fitting error rather than on wall. Offsetting the measurement
    removes both terms: the gap is the clearance, by construction, whatever shape
    the bone turns out to be.
    ...
    sample stands for rather than the single point it measured. The dilation runs
    on a grid refined ``SEAT_REFINE`` times in both directions, so the window is
    ~1 mm along the shaft rather than the ~4.5 mm station spacing -- at station
    spacing the seat clears the taper, not a bump, and pays for it in gap.
    """
...
def _smooth_envelope(seat: np.ndarray) -> np.ndarray:
    ...
    Seating straight onto sampled points makes the loft sections jagged at the
    scale of the mesh's own faceting, and neighbouring wires that wobble against
    each other loft into a solid whose faces graze and cross. That is not a
    visible failure: the STL still closes, so ``manifold_watertight`` passes, but
    a +X ray through the part comes back with one hit instead of two and the gap
    check reads a large standoff off the far wall of a fold.
    ...
    global lift is set by the worst place on the plate and spends that gap
    everywhere else, where the fit was fine.
    """
...
def _section_wire(
    ...
    corners, and filleting whatever it did pick returns an invalid solid. Building
    the relief into the profile keeps it inside the loft, where it cannot fail
    silently.
    """
    ys_out = ys
    if bevel > 0.0:
...
def build_implant(params: dict) -> cq.Workplane:
    ...
    of constant wall thickness, with six round screw bores along their planned
    trajectories.
    """
    ...
    # Seat on the cortex itself rather than on its single most protruding point.
    # A flat plate has to clear the apex of the bow and therefore gapes
    # everywhere else; contouring removes that constraint, which is what turns an
    # 8.6 mm gap into the mounting clearance.
